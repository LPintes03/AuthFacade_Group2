    ...
    <div class="bg-gradient-to-br from-red-700 to-gray-600 min-h-screen flex flex-col justify-center items-center">
        <div class="bg-white rounded-lg shadow-lg p-8 max-w-md">
            ...
            <form action="/login" method="POST">
                @csrf
                <input type="text" placeholder="Username" name="login_name" required
                ...
                @if ($errors->has('login_name'))
                    <span class="text-red-500 text-sm">{{ $errors->first('login_name') }}</span>
                @endif

                <input type="password" placeholder="Password" name="login_password" required
                    class="w-full p-3 mb-4 border border-gray-400 rounded focus:outline-none focus:ring-2 focus:ring-blue-500">
                @if ($errors->has('login_password'))
                    <span class="text-red-500 text-sm">{{ $errors->first('login_password') }}</span>
                @endif
            ...
            <a href="/register" class="toggle-link text-center mt-4 text-blue-500 cursor-pointer hover:underline">
